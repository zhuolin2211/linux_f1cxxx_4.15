			     ...
			     int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		if (ret < 0)
			return ret;
		*val = le16_to_cpu(buf[chan->address]);
		return IIO_VAL_INT;
			  ...
			  const struct i2c_device_id *id)
{
	...
	data->reset_gpiod = devm_gpiod_get_optional(&client->dev, "reset",
						    GPIOD_OUT_HIGH);
	...
	if (IS_ERR(data->avdd))
		return PTR_ERR(data->avdd);

	data->dvdd = devm_regulator_get(&client->dev, "dvdd");
	if (IS_ERR(data->dvdd))
		return PTR_ERR(data->dvdd);
