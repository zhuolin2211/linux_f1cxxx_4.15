...
static struct semaphore esp_powerup_sem;//信号量
...
int sif_spi_write_bytes(struct spi_device *spi, unsigned int addr, unsigned char *src,int count, int dummymode)
{
    ...
    sif_spi_write_async_read(spi, tx_cmd,tx_cmd,pos+len);//实际的值+补1的值
...
/* -------------------------------------------------------------------------- */
	...
	if(sif_sdio_state == ESP_SDIO_STATE_FIRST_INIT){
		...
		epub = esp_pub_alloc_mac80211(&spi->dev);//网络协议
...
/* -------------------------------------------------------------------------- */
        ...
        err = esp_pub_init_all(epub);//再发指令
...
static const struct of_device_id esp8089_id[] = {                              
	{ .compatible = "boss,esp8089" },                                     
	{},                                                                
}; 

MODULE_DEVICE_TABLE(of,esp8089_id);
...
struct spi_driver esp_spi_driver = {
	...
	.driver	= {
		...
		//.bus    = &spi_bus_type,
		//.owner	= THIS_MODULE,
        .of_match_table = of_match_ptr(esp8089_id),
		.pm = &esp_spi_pm_ops,
	},
	.probe	= esp_spi_probe,
...
static int __init esp_spi_init(void) {
  ...
  printk("DTESETS hava!\n");
    ...
    sif_platform_register_board_info();//空的
  #endif
  printk("ESP8089_init Enter\n");
  ...
  edf_ret = esp_debugfs_init(); //创建debugfs

	request_init_conf();//初始化
  ...
  do {
   ...
   sif_platform_target_poweron(); //操作reset_GPIO，硬重启
    ...
    #ifdef REGISTER_SPI_BOARD_INFO
      spi = sif_platform_new_device(); //这里手动添加的SPI_device
      if (spi) {
